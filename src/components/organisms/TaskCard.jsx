...
const TaskCard = ({ task, category, onComplete, onDelete, onUpdate, index }) => {
  ...
  return (
    <motion.div
      ...
      className={`bg-white rounded-xl p-6 shadow-sm hover:shadow-md transition-all duration-200 border-l-4 ${
        task.completed 
          ? 'opacity-60 border-l-success' 
          : category?.color === 'red' ? 'border-l-error'
          : category?.color === 'blue' ? 'border-l-info'
